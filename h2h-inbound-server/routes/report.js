...
router.get('/:perceptorid/:channelid/:terminalid/:context', function (req, res) {
  ...
  encodedRequestTokenPayload = btoa(JSON.stringify({"iss":"http://temanggungkab.go.id","sub":"REPORT","aud":"BANK","exp":helpers.currentYearExpirationDate(),"nbf":helpers.currentDateFormatNow(),"iat":"TEMANGGUNG","jti":config.jwtId,"context":context,"subcontext":"report"})),
  ...
  fetch(config.baseEndPoint+'/'+context+'/report', {
    method: "POST",
    headers: {
